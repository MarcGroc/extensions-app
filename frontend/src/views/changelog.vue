  ...
  <div class="grid grid-cols-12 gap-5">
          ...
          <div v-for="(item, i) in items" :key="i" class="mb-3">
            <Disclosure v-slot="{ open }">
              ...
              >
                <span
                  >{{ item.version
                  }}<span class="font-semibold text-xs text-slate-400"
                    >- Published on {{ item.date }}</span
                  ></span
                >
                <span
                  class="transition-all duration-150 text-xl"
                ...
                >
                  <div class="px-8 py-4">
                    <div v-for="(data, j) in item.changes" :key="j">
                      <div
                        class="flex space-x-3 items-center mt-2 text-slate-600 dark:text-slate-300 text-sm"
                      >
                        <span
                          class="h-2 w-2 bg-primary-500 rounded-full"
                        ></span>
                        <span>{{ data.name }}</span>
                        <span
                          v-if="data.tag == 'added'"
                          class="bg-indigo-100 text-indigo-500 px-2 rounded-full text-xs capitalize"
                          >{{ data.tag }}</span
                        >
                        <span
                          v-if="data.tag == 'update'"
                          class="bg-yellow-100 text-yellow-500 px-2 rounded-full text-xs capitalize"
                          >{{ data.tag }}</span
                        >
                        <span
                          v-if="data.tag == 'fixed'"
                          class="bg-red-100 text-red-500 px-2 rounded-full text-xs capitalize"
                          >{{ data.tag }}</span
                        >
                      </div>
                    </div>
    ...
    <div class="lg:col-span-4 col-span-12">
      <Card title="CHANGELOG">
        ...
        <ul
          class="space-y-3 mt-6 divide-y dark:divide-slate-700 divide-slate-100"
        >
          <li
            class="flex justify-between items-center text-xs text-slate-600 dark:text-slate-300 pt-3"
